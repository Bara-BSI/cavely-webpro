                            ...
                            <tbody>
                                @foreach ($index as $row)
                                    <tr>
                                        ...
                                        <td>
                                            @php
                                                $region = DB::table('regions')->where('id', $row->regions_id)->value('nama_region');
                                            @endphp
                                            <b>{{ $region }}</b>
                                        </td>
                                        <td>
                                            <a href="{{ route('backend.country.edit', $row->id) }}" title="Ubah Data">
                                                <button class="btn btn-cyan btn-sm"><i class="far fa-edit">Edit</i></button>
                                            </a>
                                            <form action="{{ route('backend.country.destroy', $row->id) }}" method="post" style="display: inline-block">
                                                @method('delete')
                                                @csrf
